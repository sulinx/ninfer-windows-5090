...
#if defined(_MSC_VER)
    // MSVC x64 rejects __int128 (error C4235). The result is clamped to uint64_max, and
    // min(min(L,M)+min(T,M),M) == min(L+T,M) (M = uint64_max; L and T the exact 128-bit
    // products), so the computation stays exact with saturated 64-bit operations.
    const std::uint64_t maximum = std::numeric_limits<std::uint64_t>::max();
    auto saturating_mul = [maximum](std::uint64_t a, std::uint64_t b) {
        return a != 0U && b != 0U && a > maximum / b ? maximum : a * b;
    };
    const std::uint64_t linear = saturating_mul(prefix_tokens, suffix_tokens);
    // Split suffix*(suffix+1)/2 so the /2 is exact before the product; the factor pair
    // differs by parity (odd suffix: suffix * ((suffix+1)/2), even: (suffix/2) * (suffix+1)).
    const std::uint64_t factor_a =
        (suffix_tokens & 1U) != 0U ? (suffix_tokens >> 1U) + 1U : suffix_tokens / 2U;
    const std::uint64_t factor_b =
        (suffix_tokens & 1U) != 0U ? suffix_tokens : suffix_tokens + 1U;
    const std::uint64_t triangular = saturating_mul(factor_a, factor_b);
