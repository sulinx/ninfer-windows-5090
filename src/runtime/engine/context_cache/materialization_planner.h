...
private:
    ...
    [[nodiscard]] static bool construction_option_better(const GuidanceCost& parent,
        ...
        if (a && b) {
            ...
            const auto x = delta(cost), y = delta(prior);
#if defined(_MSC_VER)
            const std::uint64_t x_hi = __umulh(x, b), x_lo = x * b;
            const std::uint64_t y_hi = __umulh(y, a), y_lo = y * a;
            if (x_hi != y_hi) { return x_hi < y_hi; }
            if (x_lo != y_lo) { return x_lo < y_lo; }
#else
            const __uint128_t left  = static_cast<__uint128_t>(x) * b;
            const __uint128_t right = static_cast<__uint128_t>(y) * a;
